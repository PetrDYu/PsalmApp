...
object SongPartBuilder {
    ...
    fun getPartBuilder(tagName: String?): PartBuilderFunc? {
        ...
        when (tagName) {
            TagAndAttrNames.VERSE_TAG._name -> {result = ::buildVerse}
            TagAndAttrNames.CHORUS_TAG._name -> {result = ::buildChorus}
            TagAndAttrNames.BRIDGE_TAG._name -> {result = ::buildBridge}
        }
        return result
    }
